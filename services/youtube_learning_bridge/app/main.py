...
app=FastAPI(title='YouTube Learning Bridge',version='0.1.1')
DATA=Path(os.getenv('DATA_DIR','./data')); (DATA/'uploads').mkdir(parents=True,exist_ok=True)
DB=DATA/'youtube_learning.jsonl'; MODEL=None
PATTERNS={
 'eq':r'\b(eq|equaliz|эквал|частот|hz|khz)\b','compression':r'\b(compress|компресс|ratio|threshold|attack|release|1176|la-2a|la2a|la-3a)\b',
 'reverb':r'\b(reverb|реверб|room|plate|hall|ambience|chamber|predelay|decay)\b','delay':r'\b(delay|slap|echo|дилей|задержк)\b',
 'saturation':r'\b(saturat|distort|clip|drive|harmonic|сатурац|клип)\b','transient':r'\b(transient|punch|панч|crest|крэст|envelope|sustain)\b',
 'stereo':r'\b(stereo|width|pan|mono|стерео|панорам)\b','loudness':r'\b(lufs|loudness|true peak|dbtp|громкост)\b',
 'drums':r'\b(kick|snare|tom|overhead|drum|барабан|малый|бочка)\b','bass':r'\b(bass|бас)\b','guitars':r'\b(guitar|гитар)\b','vocals':r'\b(vocal|voice|вокал|голос)\b'}
...
async def meta(x:str):
    ...
    if key:
        async with httpx.AsyncClient(timeout=20) as c:
            r=await c.get('https://www.googleapis.com/youtube/v3/videos',params={'part':'snippet,contentDetails','id':v,'key':key}); r.raise_for_status(); a=r.json().get('items',[])
        if a:
...
def public_transcript(x:str,languages=('ru','en')):
    ...
    seg=[{'start':float(s.start),'duration':float(s.duration),'text':str(s.text).strip()} for s in chosen.fetch() if str(s.text).strip()]
...
def whisper():
    global MODEL
    if MODEL is None: MODEL=WhisperModel(os.getenv('WHISPER_MODEL','tiny'),device=os.getenv('WHISPER_DEVICE','cpu'),compute_type=os.getenv('WHISPER_COMPUTE_TYPE','int8'))
    return MODEL

async def ingest_core(url:str,languages=('ru','en')):
    m=await meta(url)
    try:
        tr=public_transcript(url,languages); row={'metadata':m,'transcript':tr,'mixing_mentions':mentions(tr),'status':'transcript_ready'}
    except Exception as e:
        row={'metadata':m,'transcript':None,'mixing_mentions':[],'status':'needs_audio_or_transcript','reason':str(e)}
    save(row); return row

@app.get('/healthz')
def health(): return {'ok':True,'version':'0.1.1','whisper_model':os.getenv('WHISPER_MODEL','tiny')}

@app.get('/')
def home():
    return HTMLResponse("""<meta name=viewport content='width=device-width,initial-scale=1'><style>body{font-family:-apple-system;max-width:800px;margin:30px auto;padding:16px}input,button{font-size:16px;padding:10px;margin:5px 0;width:100%}pre{white-space:pre-wrap;background:#eee;padding:12px}</style><h1>YouTube Learning Bridge</h1><input id=u placeholder='YouTube URL'><button onclick='go()'>Получить транскрипт</button><pre id=o></pre><form id=f><input type=file id=x accept='audio/*,video/*'><input id=l placeholder='ru / en, необязательно'><button>Транскрибировать файл</button></form><pre id=p></pre><script>async function go(){o.textContent='...';let r=await fetch('/api/ingest?url='+encodeURIComponent(u.value));o.textContent=JSON.stringify(await r.json(),null,2)}f.onsubmit=async e=>{e.preventDefault();p.textContent='...';let d=new FormData();d.append('file',x.files[0]);d.append('language',l.value);let r=await fetch('/api/upload',{method:'POST',body:d});p.textContent=JSON.stringify(await r.json(),null,2)}</script>""")

@app.get('/api/ingest')
async def ingest_get(url:str=Query(...),languages:str='ru,en'):
    langs=tuple(x.strip() for x in languages.split(',') if x.strip()) or ('ru','en')
    return await ingest_core(url,langs)

@app.post('/api/ingest')
async def ingest_post(body:dict):
    return await ingest_core(body['url'],tuple(body.get('languages') or ['ru','en']))
...
async def upload(file:UploadFile=File(...),language:str=Form('')):
    ...
    p=DATA/'uploads'/f'{uuid.uuid4().hex}{ext}'; maxb=int(os.getenv('MAX_UPLOAD_MB','200'))*1024*1024; size=0
    try:
        with p.open('wb') as f:
        ...
        row={'uploaded_media':file.filename,'transcript':tr,'mixing_mentions':mentions(tr),'status':'transcript_ready'}; save(row); return row
