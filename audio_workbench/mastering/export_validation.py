...
mastering candidate.  It is deliberately a technical safety gate: passing it
never promotes an audio baseline or replaces human listening.
"""
...
def validate_master_exports(
    ...
    *,
    ...
    WAV and decoded lossy formats use separate true-peak tolerances because the
    codec may create modest inter-sample overs.  The tolerance is applied around
    the requested ceiling, never around a hard-coded global ceiling.

    The result is technical evidence only.  Even a fully passing delivery still
    requires human listening for subjective acceptance.
    """
    ...
    if isinstance(expected_frames, bool) or not isinstance(expected_frames, (int, np.integer)) or int(expected_frames) <= 0:
    ...
    if expected_sample_rate is not None:
        if isinstance(expected_sample_rate, bool) or not isinstance(expected_sample_rate, (int, np.integer)) or int(expected_sample_rate) < 8000:
    ...
    for fmt in p.required_formats:
        ...
        ceiling_limit = float(ceiling + tolerance)
        ...
        per_format[fmt] = {
            ...
            "frames": int(frames) if isinstance(frames, (int, np.integer)) and not isinstance(frames, bool) else None,
            "expected_frames": int(expected_frames),
            "sample_rate": int(sample_rate) if isinstance(sample_rate, (int, np.integer)) and not isinstance(sample_rate, bool) else None,
            "expected_sample_rate": int(expected_sample_rate) if expected_sample_rate is not None else None,
        }
