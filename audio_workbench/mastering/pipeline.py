...
class MasteringDirector:
    ...
    def render(self,x:np.ndarray,sr:int):
        ...
        # A sample-domain maximizer can leave intersample overshoots. Apply only
        # linked static attenuation here, then evaluate LUFS/GR on the actual output.
        # This gain is not limiter GR and must not be reported as dynamics reduction.
        safety_event=None
        if self.config.maximizer:
