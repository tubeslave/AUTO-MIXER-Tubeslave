...
def _envelope(x,sr):
    return ndimage.maximum_filter1d(np.max(np.abs(x),axis=1)+1e-12,size=max(3,int(.004*sr)),mode="nearest")

def _smooth_gain(req,sr,release_ms):
    req=ndimage.minimum_filter1d(req.astype("float32"),size=max(3,int(.004*sr)),mode="nearest")
    # Efficient release smoother suitable for long streaming renders.
...
def diagnose(x,sr,target_band_gr_db=(.65,.55,.45,.35)):
    bands=_band_split(x,sr);plan=[]
    for i,(b,tgr) in enumerate(zip(bands,target_band_gr_db)):
        e=_envelope(b,sr)
        p995=float(np.percentile(e,99.5));p90=float(np.percentile(e,90))
        crest=20*np.log10((p995+1e-12)/(p90+1e-12))
        # Set threshold from the observed upper envelope so each band works lightly.
        threshold=p995/(10**(float(tgr)/20))
        plan.append({"band":BANDS[i],"threshold":float(threshold),"target_gr_db":float(tgr),
                     "upper_envelope_crest_db":float(crest),"release_ms":RELEASE_MS[i]})
    return plan

def process(x,sr,ceiling_db=-1.0,drive_db=2.2,target_band_gr_db=(.65,.55,.45,.35)):
    driven=x*np.float32(10**(drive_db/20));bands=_band_split(driven,sr)
    ...
    # Final stage catches only reconstructed overs; target is to leave most work to bands.
    env=_envelope(out,sr);ceil=10**(ceiling_db/20);req=np.minimum(1.,ceil/env)
    g=_smooth_gain(req,sr,55.)
    out*=g[:,None]
    return out.astype("float32"),{"drive_db":drive_db,"ceiling_db":ceiling_db,"bands":stats,
