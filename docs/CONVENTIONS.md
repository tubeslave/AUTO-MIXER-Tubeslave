...
### Ayaic gain staging и баланс
- Для offline сведения, саундчека и live-режима дефолтная логика баланса —
  Ayaic Mix Monolith level planes. Они применяются не только к отдельным
  дорожкам, но и к стемам/шинам, собранным из этих дорожек.
- Рабочий порядок: выровнять слои внутри инструмента или стема, собрать stem/bus,
  измерить сумму, затем корректировать group bus. Не принимать решение по
  isolated channel, если сумма группы после панорамы/FX даёт другой баланс.
- В live-режиме Ayaic-цели не отменяют safety: fader ceiling `0 dBFS`, true peak
  headroom, feedback guard, bleed-aware trim и rate limits имеют приоритет.
- Если есть готовый `Drums Bus`, `Backs Bus`, `Guitars Bus` и т.п., измерять и
  балансировать его как самостоятельный источник; если bus-уровня нет, собрать
  виртуальный stem из входящих каналов и использовать его для решения.

| Source / group plane | Default target |
|----------------------|----------------|
| Lead/main element | -22 LUFS |
| Strong source or subgroup bus: Drums, Bass, Guitars, BG Vocals | -25 LUFS |
| Tucked backing vocals/support vocal stack | -27 LUFS |
| Keys/Synth/Pads/Ambient support | -30 LUFS |
...
Если нет более конкретного operator intent, reference recipe или сохранённой
сцены, при gain staging и балансе использовать Ayaic-таблицу выше как первичный
ориентир, а эту live-sound таблицу — как fallback для детальных инструментов до
сборки их stem/group bus.
