...
debugLog('WebSocket message received:', data.type, data);
  ...
  send(message) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        debugLog('Sending JSON string:', jsonMessage);
...
console.error(`Error in listener for ${eventType}:`, error);
  ...
  connectAbleton(ip = '127.0.0.1', sendPort = 11000, recvPort = 11001, channelOffset = 0) {
    this.send({
      type: 'connect_ableton',
      ip,
      send_port: sendPort,
      recv_port: recvPort,
  ...
  startVoiceControl(modelSize = 'small', language = 'ru', deviceId = null, channel = 0) {
    ...
    debugLog('startVoiceControl sending:', message);
    this.send(message);
  }
  ...
  startRealtimeCorrection(deviceId = null, channels = [], channelSettings = {}, channelMapping = {}, options = {}) {
    debugLog('websocketService.startRealtimeCorrection() called', {
      deviceId,
      channels,
      channelSettings,
      channelMapping,
  ...
  stopRealtimeCorrection() {
    debugLog('websocketService.stopRealtimeCorrection() called');
    this.send({
  ...
  startMultiChannelAutoEQ(deviceId, channelsConfig, mode = 'soundcheck') {
    debugLog('startMultiChannelAutoEQ called with:', { deviceId, channelsConfig, mode });
    this.send({
  ...
  startPhaseAlignment(deviceId, referenceChannel, channels, settings = {}) {
    this.send({
  ...
  startRealtimeFader(deviceId, channels, channelSettings, channelMapping, settings = {}) {
    debugLog('startRealtimeFader called with:', { deviceId, channels, channelSettings, settings });
    this.send({
  ...
  stopRealtimeFader() {
    debugLog('stopRealtimeFader called');
    this.send({
  ...
  startAutoBalance(deviceId, channels, channelSettings, channelMapping, duration = 15, bleedThreshold = -50) {
    debugLog('startAutoBalance (LEARN) called with:', { deviceId, channels, duration, bleedThreshold });
    this.send({
  ...
  applyAutoBalance() {
    debugLog('applyAutoBalance called');
    this.send({ type: 'apply_auto_balance' });
  }

  // Cancel Auto Balance collection
  cancelAutoBalance() {
    debugLog('cancelAutoBalance called');
    this.send({
  ...
  saveAutoFaderDefaults(settings) {
    debugLog('saveAutoFaderDefaults called with:', settings);
    this.send({
  ...
  loadAutoFaderDefaults() {
    debugLog('loadAutoFaderDefaults called');
    this.send({
  ...
  saveAllSettings(settings) {
    debugLog('saveAllSettings called with:', settings);
    this.send({
  ...
  loadAllSettings() {
    debugLog('loadAllSettings called');
    this.send({
  ...
  startAutoSoundcheck(deviceId, channels, channelSettings, channelMapping, timings) {
    debugLog('startAutoSoundcheck called with:', { deviceId, channels, timings });
    this.send({
  ...
  stopAutoSoundcheck() {
    debugLog('stopAutoSoundcheck called');
    this.send({
  ...
  // ========== Auto Compressor ==========
  startAutoCompressor(deviceId, channels, channelMapping, channelNames = {}, channelSettings = {}) {
    this.send({
