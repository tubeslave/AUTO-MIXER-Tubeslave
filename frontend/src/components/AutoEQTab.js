...
function detectProfileFromChannelName(channelName) {
  ...
  // Tom variations (M TOM, H TOM, L TOM, FTOM, etc.)
  if (name.includes('TOM') || name.includes('TOM-') || name.includes('TOM ') || 
      name === 'M' || name === 'H' || name === 'L' || name.startsWith('M ') || name.startsWith('H ') || name.startsWith('L ')) {
    if (name.includes('FLOOR') || name.includes('FTOM') || name.includes('F TOM') || 
        name === 'FTOM' || name === 'F TOM' || name.endsWith('FTOM')) {
      return 'ftom';
    }
    return 'tom';
  }
...
function AutoEQTab({ selectedChannels, availableChannels, selectedDevice, audioDevices }) {
  ...
  // Mode: 'soundcheck' (analyze once, then apply) or 'live' (continuous)
  const [eqMode, setEqMode] = useState('soundcheck');
  ...
  useEffect(() => {
    ...
    const handleMultiChannelSpectrum = (data) => {
      ...
      // Only set 'Analyzing' if:
      // 1. Corrections are not ready yet, AND
      // 2. Current status is not already 'Ready' or 'Applied'
      setChannelStatus(prev => {
        const currentStatus = prev[channel];
        const isReady = data.corrections_ready || channelCorrectionsReady[channel];
        if (isReady || currentStatus === 'Ready' || currentStatus === 'Applied') {
          // Keep current status - don't revert to Analyzing
          return prev;
        }
        return { ...prev, [channel]: 'Analyzing' };
      });
    ...
    const handleMultiChannelCorrections = (data) => {
      ...
      // In soundcheck mode, corrections are auto-applied, so status is 'Applied'
      // In live mode, status is 'Ready' and user can apply manually
      const isSoundcheck = eqMode === 'soundcheck';
      setChannelStatus(prev => ({ 
        ...prev, 
        [channel]: isSoundcheck ? 'Applied' : 'Ready' 
      }));
      // Mark corrections as ready for this channel
      if (data.corrections_ready) {
        setChannelCorrectionsReady(prev => ({ ...prev, [channel]: true }));
      }
    };
    ...
    const handleMultiChannelStatus = (data) => {
      if (data.active !== undefined) {
        setIsActive(data.active);
        // If soundcheck is complete, set all channels to 'Applied'
        if (!data.active && data.message && data.message.includes('Soundcheck complete')) {
          setChannelStatus(prev => {
            const newStatus = { ...prev };
            Object.keys(newStatus).forEach(ch => {
              if (newStatus[ch] === 'Analyzing' || newStatus[ch] === 'Ready') {
                newStatus[ch] = 'Applied';
              }
            });
            return newStatus;
          });
  ...
  const handleStartBatchAnalysis = () => {
    ...
    const channelsConfig = selectedChannels.map(channelId => {
      ...
      return {
        ...
        auto_apply: eqMode === 'live'  // Always auto-apply in live mode
      };
    });
    ...
    websocketService.startMultiChannelAutoEQ(deviceId, channelsConfig, eqMode);
    setStatusMessage(`Starting ${eqMode === 'soundcheck' ? 'soundcheck' : 'live'} analysis for ${channelsConfig.length} channels...`);
    setIsActive(true);
  };
      ...
      <div className="auto-eq-section">
        ...
        <div className="batch-mode-section">
            ...
            <div className="action-buttons">
              {/* Mode Selection */}
              {!isActive && (
                <div className="eq-mode-selector" style={{ marginRight: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <label style={{ fontSize: '14px', color: '#ccc' }}>Mode:</label>
                  <select 
                    value={eqMode} 
                    onChange={(e) => setEqMode(e.target.value)}
                    style={{
                      padding: '6px 12px',
                      borderRadius: '4px',
                  ...
                  >
                    <option value="soundcheck">Soundcheck (Analyze → Apply)</option>
              ...
              {!isActive ? (
                <button 
                  ...
                  title={!selectedDevice ? 'Select audio device first' : (!selectedChannels || selectedChannels.length === 0) ? 'Select channels to process first' : eqMode === 'soundcheck' ? 'Analyze once, then apply corrections' : 'Continuous analysis and auto-apply'}
                >
                  {eqMode === 'soundcheck' ? 'Start Soundcheck' : 'Start Live'}
                </button>
              ) : (
                <button 
              ...
              {/* Only show Apply button in soundcheck mode - live mode auto-applies */}
              {eqMode === 'soundcheck' && (
                <button 
