...
R3 Main PATCH_VERIFY now has three explicit proof layers:
...
- `PhysicalMainMeterEvidence` is the typed ingress for an independent Main-meter observation;
...
- `backend/live_runtime/wing_main_meter.py` now supplies a concrete read-only physical meter transport using the documented native WING metering protocol rather than a guessed OSC leaf: a short-lived TCP connection to port 2222 declares an ephemeral UDP meter return port, requests one `MAIN` collection, validates the caller report id and exact payload shape, and decodes signed big-endian level words at 1/256 dB;
- the native provider exports only the observed Main output level and receive timestamp as `PhysicalMainMeterEvidence`; it makes no WING parameter/routing writes and does not become a second persistent control authority;
- software tests validate request encoding, native escape handling, report-id/shape rejection, signed meter decoding, one-shot socket lifecycle and timeout fail-closed behavior;
- autonomous Main mutation remains blocked because hardware HIL has not yet proven that the target WING firmware returns the expected collection, that the native output detector/ballistics are sufficiently comparable to the post-console USB sample-domain evidence, or that real timestamp skew stays inside the coherence corridor.
