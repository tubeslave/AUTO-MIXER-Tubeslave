...
### 4.1. Быстрый запуск (рекомендуемый)

```bash
cd AUTO-MIXER-Tubeslave
source venv/bin/activate

# Запуск с настройками по умолчанию (dLive @ 192.168.3.70, SoundGrid)
python3 start_soundcheck.py
```

### 4.2. Запуск с параметрами
...
### 4.4. Полный список параметров CLI

```
python3 start_soundcheck.py --help

Опции:
  --mixer {dlive,wing}    Тип пульта (по умолчанию: dlive)
  --ip IP                 IP адрес пульта (по умолчанию: 192.168.3.70)
  --port PORT             Порт (по умолчанию: 51328 для dLive, 2222 для WING)
...
1. **Подключение к пульту** — соединяется с dLive по MIDI over TCP.
2. **Обнаружение аудиоустройства** — находит SoundGrid (или Dante) устройство.
