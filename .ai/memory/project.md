...
- Operator accepted Ayaic level-plane balancing as the default rule for offline mixes and real-console soundcheck/live work: gain staging and balance must measure both individual sources and the summed stems/groups. Use Ayaic planes as the primary default (`-22` lead/main, `-25` strong sources and Drums/Bass/Guitars/BG Vocal buses, `-27` tucked backing vocals, `-30` keys/synth/ambient support, `-35` FX/deep effects), while live safety limits override target loudness.
- Operator accepted the standard offline mixing pipeline as the main default: no external reference matching unless explicitly requested; use Ayaic source/stem/bus balance, Gate where useful, bounded source/drum-bus compression before final gain when crest control is needed, FX15-style section/mod space, and `spectral_ceiling_eq` as the default spectral guard. Do not use `AIR_MINUS_0_80`, final limiter, soft clipper, or master-bus compressor by default.
- Decision Engine v2 is an opt-in role layer under `automixer/`: it creates `ActionPlan` objects only; Safety Gate and Executor are separate, `decision_engine_v2.enabled` stays false by default, and `dry_run` stays true unless the operator deliberately opts into live apply.
