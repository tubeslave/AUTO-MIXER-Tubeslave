...
same mapping. No FX, mix glue, mastering or neural processing is used in this
validation context.

Measured whole-mix PerceptualSnapshot deltas:
- vocal_intelligibility: +0.017309
- foreground_db: +0.161685 dB
- harshness: +0.015378
- density: +0.002500
- width_db: -0.066569 dB
- climax_lift_db: -0.675488 dB
- punch_db: 0.0 dB

Under the repository Perceptual Critic policy this candidate is rejected:
`target_not_improved` because +0.017309 < +0.020, and `climax_regression` because
0.675488 dB > the 0.50 dB protected limit. It therefore must not receive an audition
export from the adapter and must not become a baseline.

This experiment is a controlled real-source validation context, not a claim that the
previously delivered Belye Stai mix was rebuilt or improved.
