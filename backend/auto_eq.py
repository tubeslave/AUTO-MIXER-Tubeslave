...
DEFAULT_EQ_PRESET_BASE_PATH = (
    Path(__file__).resolve().parents[1] / "presets" / "method_presets_eq.json"
)
...
class SpectrumAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"SpectrumAnalyzer initialized: {sample_rate}Hz, frame_size={frame_size}"
        )
    ...
    def _init_essentia(self):
        ...
        try:
            ...
            if not self._essentia_error_logged:
                logger.warning(
                    f"Essentia not installed: {e}. Falling back to numpy-based spectrum analysis."
                )
            ...
            if not self._essentia_error_logged:
                logger.warning(
                    f"Error initializing Essentia: {e}. Falling back to numpy-based spectrum analysis."
                )
                self._essentia_error_logged = True
            self._essentia_initialized = False
            return False
    ...
    def start(self, channel: int = 1, on_spectrum_callback: Callable = None) -> bool:
        ...
        if self._audio_capture is not None:
            try:
                ...
                self.freq_bins = np.fft.rfftfreq(
                    self.frame_size, 1.0 / self.sample_rate
                )
                ...
                logger.info(
                    f"Spectrum analyzer started via AudioCapture: ch{channel}, {self.sample_rate}Hz"
                )
                return True
            except Exception as e:
                logger.warning(
        ...
        try:
            ...
            if self.device_index is not None:
                ...
                logger.info(
                    f"Using device {self.device_index}: {device_info.get('name')}"
                )
            else:
            ...
            logger.info(
                f"Spectrum analyzer started: {self._num_channels} channels, {self.sample_rate}Hz"
            )
            return True
    ...
    def _analyze_numpy(self, audio_frame: np.ndarray) -> SpectralData:
        ...
        peak_indices, _ = find_peaks(
            spectrum, height=np.max(spectrum) * 0.1, distance=10
        )
...
class InstrumentProfiles:
    ...
    _preset_base_path: str = str(DEFAULT_EQ_PRESET_BASE_PATH)
    _preset_cache: Optional[Dict[str, Any]] = None
    _preset_cache_mtime: Optional[float] = None

    @staticmethod
    def set_preset_base_file(path: Optional[str]) -> None:
        if path:
            InstrumentProfiles._preset_base_path = str(path)
            InstrumentProfiles._preset_cache = None
            InstrumentProfiles._preset_cache_mtime = None

    @staticmethod
    def _load_external_profiles() -> Dict[str, Any]:
        path = resolve_path(
            InstrumentProfiles._preset_base_path, DEFAULT_EQ_PRESET_BASE_PATH
        )
        mtime = path.stat().st_mtime if path.exists() else None
        if (
            InstrumentProfiles._preset_cache is not None
            and InstrumentProfiles._preset_cache_mtime == mtime
        ):
            return InstrumentProfiles._preset_cache
        InstrumentProfiles._preset_cache = load_method_preset_base(
            path, expected_method="eq"
        )
        InstrumentProfiles._preset_cache_mtime = mtime
        return InstrumentProfiles._preset_cache

    @staticmethod
    def _profile_from_entry(entry: Dict[str, Any]) -> Optional[InstrumentProfile]:
        params = (
        ...
        target_curve = params.get("target_curve", [])
        cut_freq = params.get("cut_frequencies", [])
        boost_freq = params.get("boost_frequencies", [])
        if (
            not isinstance(target_curve, list)
            or not isinstance(cut_freq, list)
            or not isinstance(boost_freq, list)
        ):
            return None
        try:
            tc = [(float(a), float(b)) for a, b in target_curve]
            cf = [(float(a), float(b), float(c)) for a, b, c in cut_freq]
            bf = [(float(a), float(b), float(c)) for a, b, c in boost_freq]
            return InstrumentProfile(
                name=preset_id,
                description=str(params.get("description", preset_id)),
                target_curve=tc,
                cut_frequencies=cf,
                boost_frequencies=bf,
                low_cut_freq=float(params["low_cut_freq"])
                if params.get("low_cut_freq") is not None
                else None,
                high_cut_freq=float(params["high_cut_freq"])
                if params.get("high_cut_freq") is not None
                else None,
            )
        except Exception:
            return None
    ...
    def get_profile(instrument_type: str) -> Optional[InstrumentProfile]:
        ...
        """
        # Preferred: external unified JSON preset base.
        ext = InstrumentProfiles._load_external_profiles()
        index = ext.get("index", {}) if isinstance(ext, dict) else {}
        key = (
            str(instrument_type or "custom")
            .strip()
            .lower()
            .replace("-", "_")
            .replace(" ", "_")
        ...
        if key in index:
            profile = InstrumentProfiles._profile_from_entry(index[key])
            if profile:
                return profile
        ...
        lookup = str(instrument_type or "custom").lower()
        return profiles.get(lookup)

    @staticmethod
    def get_all_profiles() -> List[str]:
        """Get list of all available profile names."""
        ext = InstrumentProfiles._load_external_profiles()
        items = ext.get("instruments", []) if isinstance(ext, dict) else []
        if items:
            return [
                str(x.get("id")) for x in items if isinstance(x, dict) and x.get("id")
            ]
        return [
            # Drums
            "kick",
...
class EQCorrector:
    ...
    def calculate_correction(
        ...
        peak_indices, peak_props = find_peaks(
            spectrum_smooth, prominence=self.peak_prominence * 2, distance=20
        )
        ...
        for idx in peak_indices[:5]:  # Limit to top 5 resonances
            ...
            if 100 < freq < 10000:  # Only in mid-range
                ...
                if prominence > self.peak_prominence * 2:
                    ...
                    too_close = any(
                        abs(c.frequency - freq) < freq * 0.1 for c in corrections
                    )
                    if not too_close:
                        corrections.append(
                            EQBand(band_type="peak", frequency=freq, gain=gain, q=q)
                        )
    ...
    def _calculate_q_from_peak(
        self, spectrum: np.ndarray, freqs: np.ndarray, peak_idx: int
    ) -> float:
    ...
    def map_to_mixer_bands(
        self, corrections: List[EQBand]
...
# Spectral Gate removed - now using centralized bleed_service from core/
...
class AutoEQController:
    ...
    def __init__(
        ...
        self.monitored_channels: List[
            int
        ] = []  # List of channel IDs to monitor for bleeding
        self.spectrum_buffer: deque = deque(
            maxlen=int(min_analysis_time * 30)
        )  # 5 sec * 30 fps
    ...
    def start(
        ...
        success = self.analyzer.start(
            channel=channel, on_spectrum_callback=self._on_spectrum_received
        )
        ...
        if success:
            ...
            logger.info(
                f"Auto-EQ started: channel {channel}, profile {profile_name}, monitored_channels={self.monitored_channels}"
            )
        else:
    ...
    def _on_spectrum_received(self, spectral_data: SpectralData):
        ...
        elapsed_time = (
            time.time() - self.analysis_start_time if self.analysis_start_time else 0
        )
        ...
        if (
            elapsed_time >= self.min_analysis_time
            and len(self.spectrum_buffer) >= 10
    ...
    def _get_target_curve_for_visualization(self) -> List[float]:
        ...
        band_edges = np.logspace(np.log10(20), np.log10(20000), num_bands + 1)
        band_centers = [
            (band_edges[i] + band_edges[i + 1]) / 2 for i in range(num_bands)
        ]
    ...
    def _apply_cumulative_eq_to_spectrum(
        ...
        """
        if (
            not self.applied_corrections_history
            or len(self.applied_corrections_history) == 0
        ):
            return spectrum
    ...
    def _calculate_and_notify_corrections(self, spectral_data: SpectralData):
        ...
        if self.applied_corrections_history:
            compensated_spectrum = self._apply_cumulative_eq_to_spectrum(
                spectral_data.spectrum, spectral_data.frequencies
            )
        ...
        corrections = self.corrector.calculate_correction(
            compensated_spectral_data, self.current_profile, sensitivity=1.0
        )
            ...
            if bleed_info and bleed_info.bleed_ratio > 0:
                ...
                for correction in corrections:
                    if correction.gain > 0:  # Only reduce boosts, not cuts
                        ...
                        logger.debug(
                            f"Ch{self.current_channel}: Scaled boost correction at {correction.frequency} Hz "
    ...
    def apply_to_mixer(self, corrections: List[EQBand] = None) -> bool:
        ...
        if not corrections:
            logger.debug(
                "No corrections to apply (signal may be too quiet or already matches target)"
            )
            return False
        ...
        try:
            ...
            logger.info(
                "Auto-EQ apply (single): ch=%s mixer_bands=%s",
            ...
            self._notify_status(
                "applied", f"EQ applied to channel {self.current_channel}"
            )
            if type(self.mixer_client).__name__ == "AbletonClient":
                try:
                    self.mixer_client.log_eq_eight_readback(
                        self.current_channel, tag="after_apply"
                    )
                except Exception as e:
                    logger.warning("Ableton EQ readback after apply: %s", e)
            return True
    ...
    def reset_eq(self) -> bool:
        ...
        try:
            ch = int(self.current_channel)

            logger.info(f"Reset EQ: zero gains for channel {ch}")
            ...
            if type(self.mixer_client).__name__ == "AbletonClient":
                time.sleep(0.08)
                try:
                    self.mixer_client.log_eq_eight_readback(ch, tag="after_reset")
                except Exception as e:
                    logger.warning("Ableton EQ readback after reset: %s", e)
            return True
...
def room_analysis_stub(sample_rate: int = 48000) -> Dict[str, Any]:
    ...
    return {
        "implemented": False,
        "message": "Room analysis (RTA) not implemented",
        "sample_rate": sample_rate,
    }


def resolve_frequency_conflicts(
    corrections_per_channel: Dict[int, List[EQBand]],
    low_band_freq_range: Tuple[float, float] = (100.0, 200.0),
) -> Dict[int, List[EQBand]]:
    """
    ...
    """
    result = {
        ch: list(corrections) for ch, corrections in corrections_per_channel.items()
    }
    ...
    for ch, _ in boosting_channels[1:]:
        for i, b in enumerate(result[ch]):
            if low_min <= b.frequency <= low_max and b.gain > 0:
                new_gain = b.gain * 0.5
                result[ch][i] = EQBand(
                    band_type=b.band_type, frequency=b.frequency, gain=new_gain, q=b.q
                )
                logger.info(
                    f"Conflict resolver: reduced Ch{ch} {b.frequency} Hz boost to {new_gain:.1f} dB"
                )
                break
    return result
...
class MultiChannelAutoEQController:
    ...
    def start_multi_channel(
        ...
        for config in channels_config:
            ...
            logger.info(
                f"Starting analyzer for channel {channel} with profile {profile_name}, device_id={device_id}"
            )
            ...
            try:
                success = analyzer.start(
                    channel=channel, on_spectrum_callback=make_channel_callback(channel)
                )
                ...
                if success:
                    ...
                    if not profile:
                        logger.warning(
                            f"Profile '{profile_name}' not found, using 'custom'"
                        )
                    ...
                    self.active_channels[channel] = {
                        ...
                        "spectrum_buffer": deque(
                            maxlen=int(self.min_analysis_time * 30)
                        ),
                    ...
                    started_count += 1
                    logger.info(
                        f"Successfully started analysis for channel {channel} with profile {profile_name}"
                    )
                else:
                    logger.error(f"Failed to start analyzer for channel {channel}")
            except Exception as e:
                logger.error(
                    f"Exception starting analyzer for channel {channel}: {e}",
                    exc_info=True,
                )

        if started_count > 0:
            logger.info(
                f"Multi-channel Auto-EQ started for {started_count}/{len(channels_config)} channels"
            )
            return True
        else:
            logger.error(
                f"Failed to start any channel analyzers out of {len(channels_config)} requested"
            )
            return False
    ...
    def _on_channel_spectrum(self, channel: int, spectral_data: SpectralData):
        ...
        if (
            elapsed_time >= self.min_analysis_time
            and len(channel_state["spectrum_buffer"]) >= 10
            and channel_state["profile"]
        ):
    ...
    def _get_averaged_spectrum_for_channel(
        self, channel: int
    ) -> Optional[SpectralData]:
    ...
    def _calculate_and_notify_corrections(
        self, channel: int, spectral_data: SpectralData
    ):
        ...
        corrections = self.corrector.calculate_correction(
            spectral_data, profile, sensitivity=1.0
        )
            ...
            if bleed_info and bleed_info.bleed_ratio > 0:
                ...
                for correction in corrections:
                    if correction.gain > 0:  # Only reduce boosts, not cuts
                        correction.gain *= bleed_factor
                        logger.debug(
                            f"Ch{channel}: Scaled boost correction at {correction.frequency} Hz "
                            f"by {bleed_factor:.2f} due to bleed (ratio={bleed_info.bleed_ratio:.2f})"
                        )

        channel_state["corrections"] = corrections

        # Notify callback (include corrections_ready for frontend)
        if self.on_corrections_calculated:
            corrections_dict = [c.to_dict() for c in corrections]
            self.on_corrections_calculated(
                {
                    ...
                    "corrections_ready": True,
                }
            )

        # In soundcheck mode: notify when all channels ready (server will stop to avoid re-entrancy)
        if getattr(self, "_eq_mode", "soundcheck") == "soundcheck":
            all_ready = all(
                len(st.get("corrections", [])) > 0
                for st in self.active_channels.values()
            )
            if all_ready:
                logger.info("All channels ready for corrections (soundcheck complete)")
                if self.on_status_update:
                    self.on_status_update(
                        {
                            "active": False,
    ...
    def apply_channel_correction(self, channel: int) -> bool:
        ...
        if not corrections:
            logger.debug(
                f"No corrections to apply for channel {channel} (signal may be too quiet or already matches target)"
            )
            return False
        ...
        try:
            ...
            mixer_bands = self.corrector.map_to_mixer_bands(corrections)
            logger.info(
                "Auto-EQ apply (multi): ch=%s mixer_bands=%s",
                channel,
                mixer_bands,
            )
            ...
            logger.info("Applied EQ corrections to channel %s", channel)
            # Ableton: readback после multi-apply не делаем (долго: опрос Live на каждый канал).
            return True
    ...
    def apply_all_corrections(
        self, use_conflict_resolver: bool = True
    ) -> Dict[int, bool]:
    ...
    def stop_all(self):
        ...
        logger.info(
            f"Multi-channel Auto-EQ stopped. Saved corrections for {len(saved_corrections)} channels."
        )
