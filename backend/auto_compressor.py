...
class AutoCompressorController:
    ...
    async def start_soundcheck(
        ...
        for idx, audio_ch in enumerate(order):
            ...
            try:
                ...
                params = adapt_params(
                    features, profile_dict, task_type.value,
    ...
    def _estimate_gr(self, mixer_ch: int, features: ChannelSignalFeatures) -> float:
        ...
        raw_level = max(features.true_peak_db, features.peak_db, features.rms_db, features.lufs_momentary)
        ...
        if self.bleed_service and self.bleed_service.enabled:
            ...
            if audio_ch is not None:
                ...
                if bleed_info and bleed_info.bleed_ratio > 0.3:
                    ...
                    logger.debug(f"Ch{mixer_ch}: Compensated level for GR estimate: {raw_level:.1f} -> {compensated_level:.1f} dB (bleed={bleed_info.bleed_ratio:.2f})")
        ...
        if gr > GR_EXCESS_DB:
            logger.info(f"ch{mixer_ch} high GR estimate: {gr:.1f}dB (level={level:.1f}dB, thr={thr:.1f}dB, ratio={r:.1f}) -> over_compression")
        return gr
    ...
    def _maybe_correct(self, mixer_ch: int, issue: str, features: ChannelSignalFeatures):
        ...
        # Recalculate makeup gain after parameter correction
        ...
        makeup_gain = adapt_makeup(thr_new, ratio_float, rms, peak_db=peak)
