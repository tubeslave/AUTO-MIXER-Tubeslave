...
from compressor_adaptation import (
    adapt_params,
    THR_MIN,
    THR_MAX,
    THR_SAFE_MAX,
...
class AutoCompressorController:
    ...
    def start(
        ...
        channel_settings: Optional[Dict[int, Dict[str, Any]]] = None,
    ) -> bool:
        ...
        self.channel_settings = dict(channel_settings or {})
        ...
        if self._audio_capture is not None:
            try:
                ...
                for audio_ch in channels:
                    self._audio_buffers[audio_ch] = deque(
                        maxlen=max(500, int(self.sample_rate * 12 / self.chunk_size))
                    )
                    self._extractors[audio_ch] = SignalFeatureExtractor(
                        audio_ch, self.sample_rate, self.chunk_size
                    )
                self._audio_capture.subscribe(
                    "auto_compressor", self._audio_capture_poll
                )
                self.is_active = True
                time.sleep(1.0)
                logger.info(
                    f"Auto Compressor started via AudioCapture: {len(channels)} channels"
                )
                return True
            except Exception as e:
                logger.warning(
        ...
        try:
            ...
            for audio_ch in channels:
                self._audio_buffers[audio_ch] = deque(
                    maxlen=max(500, int(rate * 12 / self.chunk_size))
                )
                self._extractors[audio_ch] = SignalFeatureExtractor(
                    audio_ch, rate, self.chunk_size
                )
            ...
            time.sleep(1.0)
            logger.info(
                f"Auto Compressor started: {len(channels)} channels, stream channels={self._num_channels}, post-fader"
            )
            return True
        except Exception as e:
    ...
    def _apply_compressor_smooth(
        ...
        try:
            ...
            if compressor_on is None or compressor_on == 0:
                logger.info(
                    f"Enabling compressor on channel {mixer_ch} before applying parameters"
                )
                self.mixer_client.set_compressor_on(mixer_ch, 1)
                time.sleep(0.15)  # Increased delay for mixer to process enable command
                logger.debug(
                    f"Compressor enabled on ch{mixer_ch}, waiting before applying params"
                )
        except Exception as e:
            logger.warning(
                f"Could not check/enable compressor ch{mixer_ch}: {e}, enabling anyway"
            )
            try:
                self.mixer_client.set_compressor_on(mixer_ch, 1)
        ...
        logger.info(
            f"Applying compressor to ch{mixer_ch}: thr={thr1:.1f}dB, ratio={ratio_wing1}, attack={att1:.1f}ms, release={rel1:.1f}ms, knee={knee1:.1f}, makeup_gain={gain1:.1f}dB"
        )
        ...
        for i in range(n + 1):
            if not self.is_active:
                logger.warning(
                    f"Compressor application interrupted (is_active=False) for ch{mixer_ch}"
                )
                break
            t = _ease_out(i / n)
            ...
            try:
                ...
                logger.error(
                    f"Set compressor ch{mixer_ch} step {i}/{n}: {e}", exc_info=True
                )
            time.sleep(dt)
        ...
        logger.info(
            f"Compressor params applied to ch{mixer_ch}: thr={thr1:.1f}dB, ratio={ratio_wing1}, attack={att1:.1f}ms, release={rel1:.1f}ms, makeup_gain={gain1:.1f}dB"
        )
    ...
    async def start_soundcheck(
        ...
        logger.info(
            f"Soundcheck: audio buffers count={len(self._audio_buffers)}, stream channels={self._num_channels}, first keys={buf_keys}, order first 5={order[:5] if order else []}"
        )
        self._send_status(
            {
                "soundcheck": True,
                "message": "Soundcheck started",
        ...
        for idx, audio_ch in enumerate(order):
            ...
            name = (
                self.channel_names.get(mixer_ch, "")
                or self.channel_names.get(audio_ch, "")
                or f"Ch {mixer_ch}"
            )
            self._send_status(
                {
            ...
            if self.mixer_client and self.mixer_client.is_connected:
                try:
                    ...
                    if compressor_was_on == 1:
                        logger.info(
                            f"Soundcheck: Temporarily disabling compressor on {name} (ch{mixer_ch}) for clean signal analysis"
                        )
                        self.mixer_client.set_compressor_on(mixer_ch, 0)
                        await asyncio.sleep(
                            0.5
                        )  # Wait for compressor to fully disengage and clear any compression artifacts
                except Exception as e:
                    logger.warning(
                        f"Soundcheck: Could not disable compressor on ch{mixer_ch} for analysis: {e}"
                    )
            ...
            if buf:
                ...
                logger.debug(
                    f"Soundcheck: Cleared audio buffer for {name} before recording uncompressed signal"
                )
            if not buf:
                logger.warning(
                    f"Soundcheck: No audio buffer for audio_ch={audio_ch} (type={type(audio_ch).__name__}), mixer_ch={mixer_ch}, name={name}"
                )
                self._send_status(
                    {
                        "soundcheck": True,
                        "message": f"No audio buffer for {name}, skipping",
                        "current_channel": mixer_ch,
                    }
                )
                ...
                continue
            logger.info(
                f"Soundcheck: Recording {name} (audio_ch={audio_ch}, mixer_ch={mixer_ch}) for {self.soundcheck_duration_per_channel}s (compressor disabled for analysis)..."
            )
            samples_list = []
            start = time.time()
            while (
                time.time() - start < self.soundcheck_duration_per_channel
                and self.soundcheck_running
            ):
            ...
            logger.info(
                f"Soundcheck: Collected {len(samples_list)} blocks for {name}, total samples: {total_samples}"
            )
            if not samples_list:
                logger.warning(
                    f"Soundcheck: No audio samples collected for {name}, skipping"
                )
                self._send_status(
                    {
                        "soundcheck": True,
                        "message": f"No audio for {name}, skipping",
                        "current_channel": mixer_ch,
                    }
                )
                continue
            ...
            if samples_list:
                ...
                first_peak = (
                    np.max(np.abs(first_block)) if len(first_block) > 0 else 0.0
                )
                first_peak_db = 20 * np.log10(first_peak + 1e-10)
                logger.info(
                    f"Soundcheck: First block peak for {name}: {first_peak:.6f} linear, {first_peak_db:.1f}dB"
                )
            logger.info(
                f"Soundcheck: Analyzing {len(samples_list)} blocks for {name}..."
            )
            ...
            if features is None or features.peak_db < -100:
                logger.warning(
                    f"Soundcheck: Invalid features for {name}, using fallback"
                )
            ...
            if features.peak_db < -80 or features.lufs_momentary < -80:
                logger.warning(
                    f"Soundcheck: {name} has very low signal (peak={features.peak_db:.1f}dB, lufs={features.lufs_momentary:.1f}), may be silent or no post-fader routing"
                )
                self._send_status(
                    {
                        "soundcheck": True,
            ...
            explicit_setting = self.channel_settings.get(
                mixer_ch, {}
            ) or self.channel_settings.get(audio_ch, {})
            ...
            logger.info(
                f"Soundcheck: {name} -> instrument={instrument}, features: peak={features.peak_db:.1f}dB, true_peak={features.true_peak_db:.1f}dB, lufs={features.lufs_momentary:.1f}, rms={features.rms_db:.1f}dB, crest={features.crest_factor_db:.1f}dB"
            )
            ...
            try:
                ...
                task_type = profile_selector.detect_task_from_context(
                    ...
                    features,
                    Genre(genre.lower())
                    if genre.lower() in [g.value for g in Genre]
                    else Genre.UNKNOWN,
                    Style(style.lower())
                    if style.lower() in [s.value for s in Style]
                    else Style.LIVE,
                )
                ...
                logger.info(
                    f"Soundcheck: {name} -> profile: instrument={instrument}, task={task_type.value}, genre={genre}, style={style}, bpm={bpm}"
                )
            except Exception as e:
                logger.warning(
                    f"Error using new profile system for {name}, falling back to legacy: {e}"
                )
            ...
            logger.info(
                f"Soundcheck: {name} -> adapted params: thr={params['threshold']:.1f}dB, ratio={params['ratio_wing']}, attack={params['attack_ms']:.1f}ms, release={params['release_ms']:.1f}ms, makeup_gain={params.get('gain', 0):.1f}dB"
            )
            self._send_status(
                {
                    "soundcheck": True,
            ...
            logger.info(
                f"Soundcheck: Applying compressor params to {name} (mixer_ch={mixer_ch})..."
            )
            ...
            if compressor_was_on == 1 and self.mixer_client:
                try:
                    logger.debug(
                        f"Soundcheck: Re-enabling compressor on ch{mixer_ch} before applying new params"
                    )
                    self.mixer_client.set_compressor_on(mixer_ch, 1)
                    await asyncio.sleep(0.1)
                except Exception as e:
                    logger.warning(
                        f"Soundcheck: Could not re-enable compressor on ch{mixer_ch}: {e}"
                    )

            try:
                await loop.run_in_executor(
                    None,
                    lambda: self._apply_compressor_smooth(mixer_ch_copy, params_copy),
                )
                logger.info(f"Soundcheck: Compressor params applied to {name}")
            except Exception as e:
                logger.error(
                    f"Soundcheck: Error applying compressor to {name}: {e}",
                    exc_info=True,
                )
        ...
        self.soundcheck_running = False
        self._send_status(
            {"soundcheck": True, "complete": True, "message": "Soundcheck complete"}
        )
            ...
            try:
                ...
                if real_gr is not None:
                    ...
                    if gr_val > GR_EXCESS_DB:
                        logger.info(
                            f"ch{mixer_ch} high GR from mixer: {gr_val:.1f}dB -> over_compression"
                        )
        ...
        if self.bleed_service and self.bleed_service.enabled:
            ...
            if audio_ch is not None:
                ...
                if bleed_info and bleed_info.bleed_ratio > 0.3:
                    ...
                    compensated_level = self.bleed_service.get_compensated_level(
                        audio_ch, raw_level
                    )
    ...
    def _maybe_correct(
        self, mixer_ch: int, issue: str, features: ChannelSignalFeatures
    ):
        now = time.time()
        ...
        if n >= MAX_CORRECTIONS_IN_ROW:
            self._send_status(
                {
                    "live": True,
                    "notification": "operator_attention",
                    "channel": mixer_ch,
                }
            )
            return
        ...
        if self.bleed_service and self.bleed_service.enabled:
            ...
            if audio_ch is not None:
                ...
                if bleed_info and bleed_info.bleed_ratio > 0.5:
                    logger.debug(
                        f"Ch{mixer_ch}: Skipping correction due to high bleed (ratio={bleed_info.bleed_ratio:.2f})"
                    )
                    return
        ...
        if not params:
            logger.debug(
                f"No parameters found for ch{mixer_ch}, skipping correction. Run soundcheck first."
            )
            return
        ...
        if issue == "over":
            ...
            if gr_estimate > GR_SEVERE_DB:
                ...
                logger.warning(
                    f"Severe over-compression detected on ch{mixer_ch}: GR={gr_estimate:.1f}dB, applying aggressive correction"
                )
                ...
                if r > 2.5:
                    ...
                    params["ratio_wing"] = "2.0"
                logger.info(
                    f"Over-compression detected on ch{mixer_ch}: GR={gr_estimate:.1f}dB, applying correction"
                )
            ...
            if features.envelope_variance > 8.0:
                ...
                params["threshold"] = min(
                    THR_SAFE_MAX, params.get("threshold", thr) + 3.0
                )
                logger.info(
                    f"Pumping detected on ch{mixer_ch}, raising threshold further"
                )
            ...
            if features.crest_factor_db < 6.0 and gr_estimate > 6.0:
                ...
                params["threshold"] = min(
                    THR_SAFE_MAX, params.get("threshold", thr) + 4.0
                )
                if r > 2.0:
                    params["ratio_wing"] = "2.0"
                logger.info(
                    f"Transient loss detected on ch{mixer_ch}, reducing compression"
                )
        else:
            thr = params.get("threshold", -15)
        ...
        logger.info(
            f"Live correction ch{mixer_ch} ({issue}): thr={thr_new:.1f}dB, ratio={ratio_wing_new}, recalculated makeup_gain={makeup_gain:.1f}dB"
        )
    ...
    async def start_live(self, auto_correct: bool = True):
        ...
        if self.mixer_client and self.mixer_client.is_connected:
            logger.info(
                f"Enabling compressors for live mode on {len(self.channels)} channels"
            )
            enabled_count = 0
            for audio_ch in list(self.channels):
                mixer_ch = self.channel_mapping.get(audio_ch, audio_ch)
                try:
                    ...
                    compressor_on = self.mixer_client.get_compressor_on(mixer_ch)
                    logger.info(
                        f"Channel {mixer_ch}: current compressor_on={compressor_on} (type: {type(compressor_on).__name__})"
                    )

                    # Force enable compressor regardless of current state
                    # (state may be out of sync with mixer)
                    logger.info(
                        f"Force enabling compressor on channel {mixer_ch} for live mode"
                    )
                    self.mixer_client.set_compressor_on(mixer_ch, 1)
                    enabled_count += 1
                    ...
                    logger.warning(
                        f"Could not enable compressor ch{mixer_ch} at live start: {e}",
                        exc_info=True,
                    )
            logger.info(f"Force enabled {enabled_count} compressors for live mode")
        else:
            logger.warning(
                f"Cannot enable compressors: mixer_client={self.mixer_client is not None}, connected={self.mixer_client.is_connected if self.mixer_client else False}"
            )
