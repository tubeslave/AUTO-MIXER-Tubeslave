...
class PhaseAlignmentAnalyzer:
    ...
    def _perform_measurements(self, channel_data: Dict[int, np.ndarray]):
        ...
        for ch, signal_data in channel_data.items():
            ...
            # 1. GCC-PHAT
            delay_gcc, coherence_gcc = self._gcc_phat(ref_signal, signal_data)
            measurements['delay_gcc_ms'] = delay_gcc * 1000.0 / self.sample_rate
            measurements['coherence_gcc'] = coherence_gcc
            ...
            # Выбор оптимальной задержки (приоритет GCC-PHAT, затем IR)
            measurements['optimal_delay_ms'] = measurements['delay_gcc_ms']
            if abs(measurements['delay_ir_ms']) < abs(measurements['delay_gcc_ms']):
                measurements['optimal_delay_ms'] = measurements['delay_ir_ms']
            
            # Определение необходимости инверсии фазы
            measurements['phase_invert'] = 1 if abs(phase_diff) > np.pi / 2 else 0
    ...
    def _gcc_phat(self, ref_signal: np.ndarray, signal: np.ndarray) -> Tuple[int, float]:
        ...
        Вычисляет временную задержку между двумя сигналами с подавлением
        ...
        # FFT
        ref_fft = np.fft.fft(ref_norm)
        sig_fft = np.fft.fft(sig_norm)
        ...
        phat_weight = cross_power / (magnitude + 1e-10)
        
        # Inverse FFT
        gcc = np.fft.ifft(phat_weight)
        gcc = np.real(gcc)
        ...
        coherence_val = np.abs(cross_power).mean() / (np.abs(ref_fft).mean() * np.abs(sig_fft).mean() + 1e-10)
        
        return delay_samples, coherence_val
...
class PhaseAlignmentController:
    ...
    def start_analysis(self,
        ...
        self.analyzer = PhaseAlignmentAnalyzer(device_index=device_id)
    ...
    def apply_corrections(self, measurements: Dict = None):
        ...
        for pair_key, meas in measurements.items():
            ...
            logger.info(f"Processing channel {ch}: delay={delay_ms:.2f} ms, phase_invert={phase_invert}")
            ...
            # Применение задержки
            if abs(delay_ms) > 0.1:  # Минимальный порог 0.1ms
                ...
                # Убеждаемся, что значение в допустимом диапазоне (0.5-500 ms)
                delay_value = max(0.5, min(10.0, delay_value))
