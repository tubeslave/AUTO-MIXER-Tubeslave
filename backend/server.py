...
class AutoMixerServer:
    ...
    async def handle_client_message(self, websocket: WebSocketServerProtocol, message: str):
        try:
            ...
            logger.info(f"DEBUG: About to check conditions for msg_type='{msg_type}' (type: {type(msg_type)})")
            ...
            # Connection commands
            elif msg_type == "connect_wing":
                await self.connect_wing(data.get("ip"), data.get("send_port"), data.get("receive_port"))
                
            elif msg_type == "connect_mixing_station":
                await self.connect_mixing_station(
                    data.get("host", "127.0.0.1"),
                    data.get("osc_port", 8000),
                    data.get("rest_port", 8080)
                )
                
            elif msg_type == "discover_mixing_station":
                await self.discover_mixing_station()
                
            elif msg_type == "disconnect_wing" or msg_type == "disconnect_mixer":
                await self.disconnect_mixer()
                
            # Mixer control commands (work with any connection mode)
                ...
                q = data.get("q")
                if self.mixer_client:
                    self.mixer_client.set_eq_band(channel, band, freq, gain, q)
            ...
            # Routing commands
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if all([output_group, output_number is not None, source_group, source_channel is not None]):
                        success = self.mixer_client.route_output(
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if all([output_group, start_output is not None, num_outputs is not None, 
                        ...
                        success_count = self.mixer_client.route_multiple_outputs(
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if output_group and output_number is not None:
                        routing = self.mixer_client.get_output_routing(output_group, output_number)
                        await self.send_to_client(websocket, {
                            "type": "output_routing",
                            "routing": routing
                        })
            ...
            # Channel input routing commands
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if all([channel is not None, source_group, source_channel is not None]):
                        success = self.mixer_client.set_channel_input(channel, source_group, source_channel)
                        await self.send_to_client(websocket, {
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if all([channel is not None, source_group, source_channel is not None]):
                        success = self.mixer_client.set_channel_alt_input(channel, source_group, source_channel)
                        await self.send_to_client(websocket, {
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if channel is not None:
                        routing = self.mixer_client.get_channel_input_routing(channel)
                        await self.send_to_client(websocket, {
            ...
            # Snapshot/Scene commands
                ...
                if self.mixer_client and isinstance(self.mixer_client, WingClient):
                    ...
                    if snap_name:
                        success = self.mixer_client.save_snap(snap_name)
                        await self.send_to_client(websocket, {
            ...
            # Voice control commands
            elif msg_type == "start_voice_control":
                logger.info("=" * 60)
                ...
                try:
                    # Start voice control asynchronously
                    ...
                    # Send error response directly to this client
                    await self.send_to_client(websocket, {
                        "type": "voice_control_status",
                        "active": False,
                        "error": str(e)
            ...
            elif msg_type == "stop_voice_control":
                await self.stop_voice_control()
            elif msg_type == "rescan_channel_names":
                await self.rescan_channel_names()
                await self.send_to_client(websocket, {
                    "type": "rescan_complete",
                    "message": "Channel names rescanned successfully"
                })
            elif msg_type == "get_voice_control_status":
                await self.send_to_client(websocket, {
                    "type": "voice_control_status",
                    "active": self.voice_control.is_listening if self.voice_control else False
                })
            
            # Real-time LUFS Correction commands
            elif msg_type == "start_realtime_correction":
                logger.info("=" * 60)
                ...
                await self.start_realtime_correction(
                    ...
                    mode=data.get("mode", "lufs"),  # 'lufs' или 'peak'
                    learning_duration_sec=data.get("learning_duration_sec")
                )
            
            elif msg_type == "stop_realtime_correction":
                logger.info("=" * 60)
            ...
            elif msg_type == "get_gain_staging_status":
                await self.send_to_client(websocket, {
                    "type": "gain_staging_status",
                    **self.get_gain_staging_status()
                })
            
            elif msg_type == "update_safe_gain_settings":
                settings = data.get("settings", {})
                await self.update_safe_gain_settings(settings)
            
            # Scan channel names and recognize instruments
            elif msg_type == "scan_channel_names":
                await self.scan_and_recognize_channels(websocket, data.get("channels", []))
            
            # Scan mixer channel names for Audio Device tab
            elif msg_type == "scan_mixer_channel_names":
                logger.info("=" * 60)
                logger.info("RECEIVED scan_mixer_channel_names MESSAGE")
                logger.info(f"Data: {data}")
                logger.info("=" * 60)
                await self.scan_mixer_channel_names(websocket)
            
            # Reset TRIM to 0dB for selected channels
            elif msg_type == "reset_trim":
                await self.reset_trim(websocket, data.get("channels", []))
            
            # Bypass: Disable all modules and set faders to 0dB
            elif msg_type == "bypass_mixer":
                await self.bypass_mixer(websocket)
            
            # ========== Auto-EQ Commands ==========
            elif msg_type == "start_auto_eq":
                await self.start_auto_eq(
                    websocket,
                    device_id=data.get("device_id"),
                    channel=data.get("channel"),
            ...
            elif msg_type == "stop_auto_eq":
                await self.stop_auto_eq(websocket)
            
            elif msg_type == "set_eq_profile":
                await self.set_eq_profile(websocket, data.get("profile", "custom"))
            
            elif msg_type == "apply_eq_correction":
                await self.apply_eq_correction(websocket)
            
            elif msg_type == "reset_eq":
                await self.reset_eq(websocket, data)
            
            elif msg_type == "reset_all_eq":
                await self.reset_all_eq(websocket, data)
            
            elif msg_type == "get_eq_profiles":
                await self.send_to_client(websocket, {
                    "type": "eq_profiles",
                    "profiles": InstrumentProfiles.get_all_profiles()
                })
            
            elif msg_type == "get_auto_eq_status":
                await self.send_to_client(websocket, {
                    "type": "auto_eq_status",
                    **self.get_auto_eq_status()
                })
            
            # Multi-channel Auto-EQ commands
            elif msg_type == "start_multi_channel_auto_eq":
            ...
            elif msg_type == "stop_multi_channel_auto_eq":
                await self.stop_multi_channel_auto_eq(websocket)
            
            elif msg_type == "set_channel_profile":
                await self.set_channel_profile(
                    websocket,
                    channel=data.get("channel"),
                    profile=data.get("profile")
                )
            
            elif msg_type == "apply_channel_correction":
                await self.apply_channel_correction(
                    websocket,
                    channel=data.get("channel")
                )
            
            elif msg_type == "apply_all_corrections":
                await self.apply_all_corrections(websocket)
            
            # Phase alignment commands
            elif msg_type == "start_phase_alignment":
                await self.start_phase_alignment(
                    websocket,
                    device_id=data.get("device_id"),
            ...
            elif msg_type == "stop_phase_alignment":
                await self.stop_phase_alignment(websocket)
            
            elif msg_type == "apply_phase_corrections":
                logger.info(f"Received apply_phase_corrections command. Measurements: {data.get('measurements')}")
                await self.apply_phase_corrections(websocket, data.get("measurements"))
            
            elif msg_type == "reset_phase_delay":
                await self.reset_phase_delay(websocket, data.get("channels"))
            
            elif msg_type == "get_phase_alignment_status":
                await self.send_to_client(websocket, {
                    "type": "phase_alignment_status",
                    **self.get_phase_alignment_status()
                })
            
            # ========== Auto Fader Commands ==========
            elif msg_type == "start_realtime_fader":
                await self.start_realtime_fader(
                    websocket,
                    data.get("device_id"),
                    data.get("channels", []),
            ...
            elif msg_type == "stop_realtime_fader":
                await self.stop_realtime_fader(websocket)
            
            elif msg_type == "start_auto_balance":
            ...
            elif msg_type == "apply_auto_balance":
                await self.apply_auto_balance(websocket)
            
            elif msg_type == "cancel_auto_balance":
                await self.cancel_auto_balance(websocket)
            
            elif msg_type == "lock_channel":
                await self.lock_auto_balance_channel(websocket, data.get("channel"))
            
            elif msg_type == "unlock_channel":
                await self.unlock_auto_balance_channel(websocket, data.get("channel"))
            
            elif msg_type == "set_auto_fader_profile":
                await self.set_auto_fader_profile(websocket, data.get("profile", "custom"))
            
            elif msg_type == "update_auto_fader_settings":
                await self.update_auto_fader_settings(websocket, data.get("settings", {}))
            
            elif msg_type == "save_auto_fader_defaults":
                await self.save_auto_fader_defaults(websocket, data.get("settings", {}))
            
            elif msg_type == "load_auto_fader_defaults":
                await self.load_auto_fader_defaults(websocket)
            
            elif msg_type == "save_all_settings":
                await self.save_all_settings(websocket, data.get("settings", {}))
            
            elif msg_type == "load_all_settings":
                await self.load_all_settings(websocket)
            
            elif msg_type == "get_auto_fader_status":
                await self.send_to_client(websocket, {
                    "type": "auto_fader_status",
                    **self.get_auto_fader_status()
                })
            
            elif msg_type == "set_live_mode":
                self.live_mode = bool(data.get("enabled", False))
                if self.gain_staging:
                    self.gain_staging.apply_preset("live" if self.live_mode else "soundcheck")
                await self.send_to_client(websocket, {"type": "live_mode_set", "live_mode": self.live_mode})
            
            elif msg_type == "emergency_stop":
                # Mute all automation: freeze gain staging and auto fader
                if self.gain_staging:
                    self.gain_staging.automation_frozen = True
                if self.auto_fader_controller:
                    self.auto_fader_controller.set_automation_frozen(True)
                if getattr(self, "auto_fader_v2_controller", None):
                    self.auto_fader_v2_controller.set_automation_frozen(True)
                await self.send_to_client(websocket, {"type": "emergency_stop_applied"})
            
            elif msg_type == "set_automation_frozen":
                frozen = bool(data.get("frozen", False))
                if self.auto_fader_controller:
                    self.auto_fader_controller.set_automation_frozen(frozen)
                if self.gain_staging:
                    self.gain_staging.automation_frozen = frozen
                if getattr(self, "auto_fader_v2_controller", None):
                    self.auto_fader_v2_controller.set_automation_frozen(frozen)
                await self.send_to_client(websocket, {"type": "automation_frozen_set", "frozen": frozen})
            
            elif msg_type == "set_channel_frozen":
                ch = data.get("channel")
                seconds = float(data.get("seconds", 10))
                if ch is not None and self.auto_fader_controller:
                    self.auto_fader_controller.set_channel_frozen(int(ch), seconds)
                if getattr(self, "auto_fader_v2_controller", None) and ch is not None:
                    self.auto_fader_v2_controller.set_channel_frozen(int(ch), seconds)
                await self.send_to_client(websocket, {"type": "channel_frozen_set", "channel": ch, "seconds": seconds})
            
            elif msg_type == "get_freeze_status":
                status = {"live_mode": self.live_mode}
                if self.auto_fader_controller:
                    status["auto_fader"] = self.auto_fader_controller.get_freeze_status()
                if getattr(self, "auto_fader_v2_controller", None):
                    status["auto_fader_v2"] = self.auto_fader_v2_controller.get_freeze_status()
                if self.gain_staging:
                    status["gain_staging_frozen"] = getattr(self.gain_staging, "automation_frozen", False)
                await self.send_to_client(websocket, {"type": "freeze_status", **status})
            
            elif msg_type == "get_ready_for_live":
                await self.send_to_client(websocket, {"type": "ready_for_live", **self.get_ready_for_live_status()})
            
            elif msg_type == "create_snapshot" or msg_type == "undo_snapshot_create":
                await self.create_snapshot(websocket, data.get("channels"))
            
            elif msg_type == "undo_restore_snapshot":
                await self.restore_snapshot(websocket, data.get("snapshot_path"))
            
            # ========== Auto Soundcheck Commands ==========
            elif msg_type == "start_auto_soundcheck":
                await self.start_auto_soundcheck(
                    websocket,
                    data.get("device_id"),
                    data.get("channels", []),
                    data.get("channel_settings", {}),
            ...
            elif msg_type == "stop_auto_soundcheck":
                await self.stop_auto_soundcheck(websocket)
            
            elif msg_type == "get_auto_soundcheck_status":
                await self.send_to_client(websocket, {
                    "type": "auto_soundcheck_status",
                    "is_running": self.auto_soundcheck_running,
                    "current_step": None,
                    "step_progress": 0,
                    "step_time_remaining": 0,
                    "message": "Idle" if not self.auto_soundcheck_running else "Running"
                })
            
            # ========== Auto Compressor Commands ==========
            elif msg_type == "start_auto_compressor":
                await self.start_auto_compressor(
                    websocket,
                    data.get("device_id"),
                    data.get("channels", []),
                    data.get("channel_mapping", {}),
            ...
            elif msg_type == "stop_auto_compressor":
                await self.stop_auto_compressor(websocket)
            elif msg_type == "get_auto_compressor_status":
                await self.get_auto_compressor_status(websocket)
            elif msg_type == "start_auto_compressor_soundcheck":
                await self.start_auto_compressor_soundcheck(
                    websocket,
            ...
            elif msg_type == "stop_auto_compressor_soundcheck":
                await self.stop_auto_compressor_soundcheck(websocket)
            elif msg_type == "start_auto_compressor_live":
                await self.start_auto_compressor_live(websocket, data.get("auto_correct", True))
            elif msg_type == "stop_auto_compressor_live":
                await self.stop_auto_compressor_live(websocket)
            elif msg_type == "set_auto_compressor_profile":
                await self.set_auto_compressor_profile(websocket, data.get("channel"), data.get("profile", "base"))
            elif msg_type == "set_auto_compressor_manual":
                await self.set_auto_compressor_manual(websocket, data.get("channel"), data.get("params", {}))
    ...
    async def start_multi_channel_auto_eq(self, websocket, device_id: str = None, channels_config: List[Dict] = None):
        """Start multi-channel Auto-EQ analysis."""
        channels_config = channels_config or []
        logger.info(f"Starting Multi-Channel Auto-EQ: device={device_id}, channels={len(channels_config)}")
    ...
    async def _apply_phase_corrections_direct(self, measurements: dict):
        ...
        for pair_key, meas in measurements.items():
            ...
            logger.info(f"Applying corrections to channel {ch}: delay={delay_ms:.2f} ms, phase_invert={phase_invert}")
            
            # Применение задержки
            if abs(delay_ms) > 0.1:
                delay_value = max(0.5, min(10.0, abs(delay_ms)))
                self.mixer_client.set_channel_delay(ch, delay_value, mode="MS")
                logger.info(f"Channel {ch}: Applied delay {delay_value:.2f} ms (clamped to max 10ms)")
            else:
                self.mixer_client.send(f"/ch/{ch}/in/set/dlyon", 0)
                logger.info(f"Channel {ch}: Delay disabled")
    ...
    async def get_auto_compressor_status(self, websocket):
        """Return current Auto Compressor status."""
        if not self.auto_compressor_controller:
            await self.send_to_client(websocket, {"type": "auto_compressor_status", "active": False})
            return
