...
if "lufs_gain_staging" in imported:
    ...
    try:
        ...
        source = open(os.path.join(BACKEND_DIR, "lufs_gain_staging.py")).read()
...
if "auto_eq" in imported:
    ...
    try:
        ...
        source_eq = open(os.path.join(BACKEND_DIR, "auto_eq.py")).read()
...
if "auto_compressor" in imported:
    auto_comp = imported["auto_compressor"]
    try:
        source_comp = open(os.path.join(BACKEND_DIR, "auto_compressor.py")).read()
        
        # 4.1: Проверяем разумные диапазоны ratio
...
if "auto_gate_caig" in imported:
    ...
    try:
        source_gate = open(os.path.join(BACKEND_DIR, "auto_gate_caig.py")).read()
...
if "auto_phase_gcc_phat" in imported:
    ...
    try:
        source_phase = open(os.path.join(BACKEND_DIR, "auto_phase_gcc_phat.py")).read()
...
if "auto_fader" in imported:
    try:
        source_fader = open(os.path.join(BACKEND_DIR, "auto_fader.py")).read()
...
if "auto_effects" in imported:
    try:
        source_fx = open(os.path.join(BACKEND_DIR, "auto_effects.py")).read()
...
if "wing_client" in imported:
    try:
        source_wing = open(os.path.join(BACKEND_DIR, "wing_client.py")).read()
...
if "bleed_service" in imported:
    try:
        source_bleed = open(os.path.join(BACKEND_DIR, "bleed_service.py")).read()
...
if "cross_adaptive_eq" in imported:
    try:
        source_xeq = open(os.path.join(BACKEND_DIR, "cross_adaptive_eq.py")).read()
...
if "auto_panner" in imported:
    try:
        source_pan = open(os.path.join(BACKEND_DIR, "auto_panner.py")).read()
...
if "auto_reverb" in imported:
    try:
        source_rev = open(os.path.join(BACKEND_DIR, "auto_reverb.py")).read()
...
with open(os.path.join(BACKEND_DIR, "test_results.json"), "w") as f:
