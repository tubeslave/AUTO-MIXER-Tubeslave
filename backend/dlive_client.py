...
def build_nrpn(channel: int, msb: int, lsb: int, value: int) -> bytes:
    ...
    return bytes(
        [
            ...
            0xB0 | ch,
            0x06,
            val_msb,  # Data Entry MSB
            0xB0 | ch,
            0x26,
            val_lsb,  # Data Entry LSB
        ]
    )
...
def build_program_change(
    channel: int, bank_msb: int, bank_lsb: int, program: int
) -> bytes:
...
class DLiveClient(MixerClientBase):
    ...
    def __init__(
        ...
        logger.info(
            f"DLiveClient initialized for {ip}:{port} (TLS={tls}, base_ch={midi_base_channel})"
        )
    ...
    def connect(self, timeout: float = 5.0) -> bool:
        ...
        try:
            ...
            # Start auto-reconnect daemon thread
    ...
    def _receiver_loop(self):
        ...
        # Auto-reconnect handled by _auto_reconnect_loop daemon thread
        if not self._stop_event.is_set():
            logger.info("dLive connection lost, waiting for auto-reconnect...")

    def _auto_reconnect_loop(self):
        """Daemon thread for automatic reconnection."""
        while not self._stop_event.is_set():
            time.sleep(self._reconnect_interval)
            if self._stop_event.is_set():
                break
            if not self.is_connected:
                logger.info(f"dLive attempting to reconnect...")
                try:
                    self.connect()
                    if self.is_connected:
                        logger.info("dLive reconnected successfully")
                        break
                except Exception as e:
                    logger.warning(f"dLive reconnect failed: {e}")
    ...
    def _midi_channel(self, channel_type: str = "input") -> int:
        return (
            self.midi_base_channel + CHANNEL_TYPE_OFFSET.get(channel_type, 0)
        ) & 0x0F
    ...
    def set_mute(self, channel: int, muted: bool, channel_type: str = "input"):
        ...
        msg = build_note_on(midi_ch, note, velocity) + build_note_on(
            midi_ch, note, 0x00
        )
        self._send_raw(msg)
    ...
    def set_channel_colour(
        self, channel: int, colour: str, channel_type: str = "input"
    ):
        midi_ch = self._midi_channel(channel_type)
